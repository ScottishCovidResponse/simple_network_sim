...
def download_pop_table():
    """
    Download the population data from an external source, using only the Area1 data and the first 3 columns
    ...
    with zipfile.ZipFile(zip_filename, 'r') as zip_ref:
        zip_ref.extractall(".", members=[population_table])
    ...
    Path(population_table).unlink(missing_ok=False)
...
def download_google_mogility_data(la_list):
    """
    Downloads the mobility data from google for Scottish local authorities
    :param: The list of local åuthorities ﬅor which we want the movement data
    :return: A dataframe containing the full ISO code (GB-iso_3166_2) and the corresponding local authority code
    """
    url = "https://www.gstatic.com/covid19/mobility/Global_Mobility_Report.csv"
    google_mobility_table = "Global_Mobility_Report.csv"
    urllib.request.urlretrieve(
            url, google_mobility_table
            )
    ...
    # clean up (i.e. remove) the downloaded datafile(s)
    Path(google_mobility_table).unlink(missing_ok=False)
